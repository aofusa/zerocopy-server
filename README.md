...
- **kTLS**: s2n-tls 使用時はカーネルTLSオフロード対応（Linux 5.15+）
...
# s2n-tls + kTLSサポート付きビルド
cargo build --release --features ktls
```

## TLS証明書の生成

開発・テスト用の自己署名証明書を生成するには、以下のコマンドを実行します（ED25519使用）：
...
### s2n-tls（`--features ktls`）

- AWSが開発した高性能TLS実装
- **kTLSをネイティブサポート**
- TLSハンドシェイク完了後、自動的にkTLSを有効化
- s2n-tls-sysのビルドにcmakeとlibcrypto-devが必要
...
| セッションキー露出 | TLSハンドシェイクはユーザースペース（rustls/s2n-tls）で実行（PFS維持） |
